...
deming.estim <- function(x,y,lambda=1){  # lambda=sigmayÂ²/sigmaxÂ²
  ...
  A <- sqrt((SSDy - lambda*SSDx)^2 + 4*lambda*SPDxy^2)
    ...
    # (c'est-Ã -dire x et y fort corrÃ©lÃ©s)
    # donc beta peu sensible Ã  lambda 
  B <- SSDy - lambda*SSDx
  beta <- (B + A) / (2*SPDxy)
  alpha <- my - mx*beta
  ...
  ) # seul sigma est sensible Ã  lambda dans cas corrÃ©lÃ©
}
...
deming.predict <-  function(x, y, lambda=1, xnew,  level=95/100){
  ...
  # see http://stats.stackexchange.com/questions/6163/what-is-the-prediction-error-while-using-deming-regression-weighted-total-least
  ...
  sd.ynew.Fuller.diff <- sqrt( sigma.ee + t(Xnew)%*%V%*%Xnew + sigma.uu) # corriger sigma.uu et sigma.ee ?
